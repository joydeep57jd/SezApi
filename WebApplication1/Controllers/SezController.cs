...
namespace SezApi.Controllers
{
    ...
    public class SezController : Controller
    {
        ...
        public async Task<ActionResult<List<CustomAppraisementApplicationHeader>>> GetCustomAppraisementApplicationHeader(int? id, int? page, int? size, bool? isInvoiceCheck)
        {
		...
		[HttpGet("GetContainerlistByGetEntry")]
		public async Task<IActionResult> GetContainerlistByGetEntry()
		{
			try
			{
				var response = await _services.GetContainerlistByGetEntry();
				return Ok(response);
			}
